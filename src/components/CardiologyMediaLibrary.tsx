...
import { displayTitleFor, sourceNameFor, type CardiologyMedia } from '../lib/cardiology-media';
...
export default function CardiologyMediaLibrary({items}:{items:CardiologyMedia[]}) {
  ...
  return <div>
    <div class="mb-5 grid gap-2 sm:grid-cols-3" aria-label="How to use this library">
      {['1 · Find a study','2 · Watch the full loop','3 · Work through the read'].map((label,i)=><div class="rounded-xl border border-clinical-200 bg-white/60 px-4 py-3 text-sm dark:border-clinical-700 dark:bg-clinical-800/60"><strong class="text-red-800 dark:text-red-300">{label}</strong><span class="mt-1 block text-clinical-500">{i===0?'Search by diagnosis, anatomy, or modality.':i===1?'Open a dedicated, mobile-safe study page.':'Use the systematic interpretation and pitfalls.'}</span></div>)}
    </div>
    ...
    <div class="mt-6 columns-1 gap-4 sm:columns-2 lg:columns-3">{filtered.map(item=><a href={`/media/${item.id}`} class="mb-4 block w-full break-inside-avoid overflow-hidden rounded-xl border border-clinical-200 bg-warm-white text-left transition hover:-translate-y-0.5 hover:border-red-500 hover:shadow-md focus-visible:outline-2 focus-visible:outline-offset-2 focus-visible:outline-red-700 dark:border-clinical-700 dark:bg-clinical-800">
      ...
      <span class="block p-4"><span class="text-xs font-semibold uppercase tracking-wide text-red-700 dark:text-red-300">{item.modality} · {item.mediaKind==='video'?'Cine loop':'Still image'}</span><span class="mt-1 block font-heading text-lg font-semibold leading-snug">{displayTitleFor(item)}</span><span class="mt-2 line-clamp-3 block text-sm leading-6 text-clinical-600 dark:text-clinical-300">{item.description}</span><span class="mt-3 flex items-center justify-between gap-3 text-sm"><span class="truncate text-clinical-400">{sourceNameFor(item)}</span><strong class="shrink-0 text-red-700 dark:text-red-300">Study →</strong></span></span>
    </a>)}</div>
  </div>
}
